<template>
  <div class="public">
    <van-nav-bar :title="$t('safe.cashWay.weChat.tit')" left-arrow @click-left="back" />
    <div class="content">
      <div class="login_info">
        <van-field  v-model="name" clearable :placeholder="$t('safe.cashWay.weChat.w1')" />
        <van-field  v-model="num" clearable :placeholder="$t('safe.cashWay.weChat.w1')" />
        <div class="add_code">
          <h4>{{$t('safe.cashWay.weChat.w3')}}</h4>
          <div class="code_img">
      ...
      <van-button type="primary" size="large" @click="addWeChat">{{$t('safe.cashWay.weChat.btn')}}</van-button>
    </div>
  ...
  export default {
    ...
    methods: {
        ...
        that
          ...
          .then(function(res) {
            if (res.data.status == 1) {
              ...
              that.we_info = res.data.data.we_info;
              that.name =  that.we_info.wechat_name;
              that.num = that.we_info.wechat_num;
              that.pay = that.we_info.wechat_pay;
      ...
      addWeChat() {
        ...
        if (!name || name == null) {
           that.$toast(that.$t('safe.cashWay.weChat.w1'));
        } else if (!num || num == null) {
          that.$toast(that.$t('safe.cashWay.weChat.w2'));
        } else if (!pay || pay == null) {
          that.$toast(that.$t('safe.cashWay.weChat.w4'));
